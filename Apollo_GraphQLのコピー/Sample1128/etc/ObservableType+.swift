...
extension ObservableType {
    ...
    func showLoading(_ isLoadingRelay: PublishRelay<Bool>) -> Observable<Element> {
        map {
    ...
    func hideLoading(_ isLoadingRelay: PublishRelay<Bool>) -> Observable<Element> {
        map {
    ...
    public func flatMapLatest<Source>(
        _ isLoadingRelay: PublishRelay<Bool>,
        _ selector: @escaping (Self.Element) throws -> Source
    ) -> RxSwift.Observable<Source.Element> where Source: RxSwift.ObservableConvertibleType {
        showLoading(isLoadingRelay)
            .flatMapLatest(selector)
            .hideLoading(isLoadingRelay)
    }
